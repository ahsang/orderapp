...
class Auth_admin extends CI_Controller {
    ...
    function __construct() 
    {
		...
		$this->data['base_url']= 'http://'.$_SERVER['HTTP_HOST']."/";
		$this->data['includes_dir']= 'http://'.$_SERVER['HTTP_HOST'].'/includes/';
	...
	*/
	...
	function orders()
    {
		...
		$this->load->view('demo/admin_examples/order_dash', $this->data);
	}
...
// 		$this->data['categories']=$this->med_model->get_batch();
		...
		$this->load->view('demo/admin_examples/add_med', $this->data);
	}
	...
	function batch_update_execute(){
		...
		foreach ($temp as $t){
			if($t->CategoryID==$input['CategoryID'] && $t->Size==$input['Size']){
				$this->db->where('Entry_ID',$t->Entry_ID);
				$this->db->update('PackingMedJoin', $input_db);
				$count+=1;
			}
	...
	function details()
	{
		...
		$this->load->view('demo/admin_examples/med_views', $this->data);
	}
	...
	function order_details()
	{
		...
		$this->data['order_details']=$this->med_model->get_order_details($id);
		//var_dump($this->data['medicine_details']);
		
		$this->load->view('demo/admin_examples/order_view', $this->data);
	}
